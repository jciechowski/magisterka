...
\begin{Definicja}
  ...
  \end{figure} ormal{K-kolorowanie grafu} to przyporządkowanie takiej liczby naturalnej każdemu wierzchołkowi grafu G, że żadnym dwóm sąsiednim wierzchołkom nie została przypisana taka sama liczba oraz wszystkie liczby są mniejsze bądź równe K.
\end{Definicja}
...
\indent Niech \textit{P'} będzie wielokątem uzyskany według powyższej procedury. W takim wypadku \textit{P'} posiada \textit{n + 2h} krawędzi oraz \textit{t} trójkątów, ponieważ każde cięcie daje nam dwie dodatkowe krawędzie ale nie dodaje nowych trójkątów. Zgodnie z tw. Chv\'atala \textit{P'} potrzebuje $\floor{\frac{n+2h}{3}} = \ceil{\frac{t}{3}}$ strażników.
