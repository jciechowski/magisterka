...
\begin{proof}
  ...
  \end{figure} 
\end{proof}
	Strażników rozmieszczamy w tych wierzchołkach, których kolor jest najrzadziej używanym. Zauważmy, że kolor ten występuję co najwyżej $\floor{\frac{n}{3}}$ razy. \todo[inline]{dlaczego takie rozmieszczenie gwarantuje pokrycie? zasad szufladkowa dirichleta?}
...
Przez kratę rozumiemy graf $G_{n,m}$, gdzie $n$ to liczba wierzchołków, a $m$ liczba krawędzi, z których każda ma długość jeden. Zakładamy również, że graf jest rozpięty na płaszczyźnie z punktem poczatkowym $v_{0,0}$ w lewym górnym wierzchołku. \todo[inline]{zła definicja}
\begin{figure}[ht!]
   \centering
...
\begin{Twierdzenie} \cite{poscig}
	Jeżeli $S_c \le n + 1$, policjant zawsze wygrywa dla stałej $S_n = n + 1$. \todo[inline]{złe twierdzenie}
\end{Twierdzenie}
\begin{proof}
...
W ostatnim rozdziale zajmuje się problemem pościgu i ucieczki w grafie, przedstawiając wariant klasyczny zaproponowany przez Sugiharu i Suzuki. Postępując zgodnie z przedstawionym przez \todo[inline]{przez kogo?} algorytmem policjant, poruszający się z prędkością $S_n = n + 1$, zawsze jest w stanie dostrzec złodzieja którego prędkość nie przekracza $S_c \leq n + 1$. \todo[inline]{jaka prędkość tutaj?}
...
\begin{thebibliography}{99}
 ...
 \newblock{{\em Discrete Applied Mathematics} str. 149–157 (1994).}
